...
namespace msIntereface
{
    ...
    public class MsApi : MsIntereface {
        public async Task<DocumentSignOperaation> PostDocumentSignOperaation(DocumentSignOperaation op) {return await Task.FromResult(new DocumentSignOperaation());}
        public async Task<TransactionOperaation> PostTransactionOperaation(TransactionOperaation op) {return await Task.FromResult(new TransactionOperaation());}
        public async Task<CreditOperaation> PostCreditOperaation(CreditOperaation op)  { return await Task.FromResult(new CreditOperaation());}   
        public async Task<CreditOperaation> NewCreditOperaation()  { return await Task.FromResult(new CreditOperaation());}   
        public async Task<TransactionOperaation> NewTransactionOperaation()  { return await Task.FromResult(new TransactionOperaation());}   
        public async Task<DocumentSignOperaation> NewDocumentSignOperaation()  { return await Task.FromResult(new DocumentSignOperaation());}   
        public async Task<List<TransactionOperaation>> GetTransactionOperaation(int account_id)  { return await Task.FromResult(new List<TransactionOperaation>());}   
        public async Task<List<TransactionOperaation>> GetTransactionOperaationJ(int op_id, int count) { return await Task.FromResult(new List<TransactionOperaation>());}   
        public async Task<List<TransactionOperaation>> DeleteTransactionOperaationJ(int op_id, int count) { return await Task.FromResult(new List<TransactionOperaation>());}   
        public async Task<List<CreditOperaation>> GetCreditOperaation(int user_id) { return await Task.FromResult(new List<CreditOperaation>());}   
        public async Task<List<CreditOperaation>> GetCreditOperaationJ(int op_id, int count) { return await Task.FromResult(new List<CreditOperaation>());}
        public async Task<List<CreditOperaation>> DeleteCreditOperaationJ(int op_id, int count) {
        ...
        try
            {
                ...
                using (var reader = cmd.ExecuteReader()) {
                    while(reader.Read()) {
                        ...
                        temp.status.status_id = reader.GetInt32(2);
                        temp.status.whoAprovedUserId = reader.GetInt32(3);
                        temp.status.created_dt = reader.GetDateTime(4);
                        temp.status.edit_dt = reader.GetDateTime(5);
                        temp.acount_id = reader.GetInt32(6);
                        temp.period = reader.GetDateTime(7);
                        temp.currency = reader.GetString(8);
                        temp.amount = reader.GetInt64(9);
                           using (var command = new NpgsqlCommand("DELETE FROM CreditOperaation WHERE id ( IN SELECT " 
        ...
        public async Task<List<DocumentSignOperaation>> GetDocumentSignOperaations(int user_id) { 
            ...
            try
            {
                ...
                using (var reader = cmd.ExecuteReader()) {
                    while(reader.Read()) {
                        ...
                        temp.status.created_dt = reader.GetDateTime(4);
                        temp.status.edit_dt = reader.GetDateTime(5);
                        temp.document_id = reader.GetInt32(6);
                        res.Add(temp);
                    }
                }
                conn.Close();
         ...
         try
            {
                ...
                using (var cmd = new NpgsqlCommand("select account_id, amount, from accountmeta where account_id=" + account_id, conn))
                using (var reader = cmd.ExecuteReader()) {
                    while(reader.Read()) {
                        ...
                        if (amount < 0)
                            ...
                            res = true;
                            using (var command = new NpgsqlCommand("UPDATE  SET amount = " + (amount + temp.amount)  + " WHERE name = " + account_id, conn)) {
                            }
                        }
                    }
                conn.Close();
        ...
        public async Task<Boolean> withdraw(int account_id, long amount) {
         Boolean res = false;
            try
            {
                ...
                using (var cmd = new NpgsqlCommand("select user_id, amount, from accountmeta where user_id=" + account_id, conn))
                using (var reader = cmd.ExecuteReader()) {
                    while(reader.Read()) {
                        AccountMeta temp = new AccountMeta();
                        temp.id = reader.GetInt32(0);
        ...
        public async Task<List<UserMeta>> getUsers(int first, int count) {
            ...
            try
            {
                ...
                using (var cmd = new NpgsqlCommand("select id, name, user_id, amount, frozen from accountmeta where user_id=" + first, conn))
                using (var reader = cmd.ExecuteReader()) {
                    while(reader.Read()) {
                        UserMeta temp = new UserMeta();
                        temp.id = reader.GetInt32(0);
                        temp.name = reader.GetString(1);
                        temp.priv = reader.GetInt32(2);
                        res.Add(temp);
                    }
                }
                conn.Close();
